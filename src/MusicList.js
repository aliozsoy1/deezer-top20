...
const MusicList = () => {
  ...
  useEffect(() => {
    const fetchMusicData = async () => {
      try {
        ...
        const response1 = await axios.get('https://cors-anywhere.herokuapp.com/https://api.deezer.com/chart?limit=10');
        const data1 = response1.data.tracks.data;
        document.title = "Deezer En İyi 20 Şarkı";
        // Sonraki 10 şarkıyı alıyoruz
        const response2 = await axios.get('https://cors-anywhere.herokuapp.com/https://api.deezer.com/chart?limit=10&index=10');
  ...
  const getSongPageUrl = async (songId) => {
    try {
      const response = await axios.get(`https://cors-anywhere.herokuapp.com/https://api.deezer.com/track/${songId}`);
